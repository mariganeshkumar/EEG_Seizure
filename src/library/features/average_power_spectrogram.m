...
function [output] = compute_average_power_spectrogram(input, sampling_rate, winSize, overlap, low, high, nFFT)
% INPUT
% input = A matrix having No of Channels*time
...
aps = [];
for i=1:m
    [s,f,~] = spectrogram(input(i,:), winSize, overlap,[low:(sampling_rate/nFFT):high], sampling_rate);
    s = abs(s);
    aps(i,:,:)  = s;
end
output = aps;
